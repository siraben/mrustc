...
| C codegen + link (M9-06) | **core-linked executable green** (`27a6b8eb`) | u-MIR C emitter: 28/28 no_core fixtures (operators, aggregates/enums, stores, references, loops, match, generic instances + trait dispatch, closures, default methods, body-local traits/externs) as a required gcc+tcc gate; `run_umir_core.sh` compiles `core-max-probe` (core::cmp::max + Option match) against the real 1.90.0 core, links, and runs — first program executing real core code |
