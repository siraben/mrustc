...
fn tail_len(b: &Buf, n: usize) -> usize {
    b[n..].len()
}

fn tail_first(b: &Buf, n: usize) -> u32 {
    let t: &[u32] = &b[n..];
    t[0]
}

fn element(b: &Buf, i: usize) -> u32 {
    b[i]
}

#[no_mangle]
pub extern "C" fn vec_index(x: u32) -> u32 {
    let b = Buf { data: [x, x + 1, x + 2, 99], len: 3 };
    tail_len(&b, 1) as u32 * 100 + tail_first(&b, 2) + element(&b, 1) * 1000
}
