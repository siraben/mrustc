...
static void test_gat_bound_generic_scope_model_invariants(void)
{
    CmHirContext context;
    CmHirCrateId crate_id;
    CmHirModuleId root_id;
    CmHirDefId bound_definition;
    CmHirDefId bound_item_definition;
    ...
    CmHirGenericParam parameter;
    CmHirGenericParamId bound_parameter_start;
    CmHirGenericParamId parameter_id;
    CmHirGenericParamId owner_parameter_start;
    CmHirGenericParamId gat_parameter_start;
    CmHirGenericParamId unrelated_parameter;
    CmHirGenericParamId mismatched_parameter;
    CmHirType type;
    CmHirTypeId usize_type;
    CmHirTypeId u8_type;
    CmHirTypeId owner_type;
    CmHirTypeId gat_type;
    CmHirTypeId unrelated_type;
    CmHirGenericArg arguments[6];
    CmHirAssociatedTypeBound bound;
    CmHirAssociatedTypeEquality equality;
    CmHirItem item;
    CmHirItemId item_id;
    CmHirItemId gat_item_id;
    const CmHirItem *stored;
    const CmHirGenericParam *stored_parameter;
    const CmHirType *stored_type;
    size_t item_count;
    size_t arena_bytes;

    cm_hir_context_init(&context);
    assert(cm_hir_create_crate(&context,
        cm_hir_intern(&context, "gat_bound_scope"), CM_HIR_EDITION_2024,
        test_span(0u, 500u), &crate_id, &root_id) == CM_HIR_OK);
    memset(&type, 0, sizeof(type));
    type.kind = CM_HIR_TYPE_INTEGER_KIND;
    ...
    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TRAIT, test_span(10u, 80u), &bound_definition)
        == CM_HIR_OK);
    memset(&parameter, 0, sizeof(parameter));
    parameter.kind = CM_HIR_GENERIC_LIFETIME;
    parameter.owner = bound_definition;
    parameter.name = cm_hir_intern(&context, "x");
    parameter.span = test_span(12u, 13u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &bound_parameter_start) == CM_HIR_OK);
    parameter.index = 1u;
    parameter.name = cm_hir_intern(&context, "y");
    parameter.span = test_span(14u, 15u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    parameter.kind = CM_HIR_GENERIC_TYPE;
    parameter.index = 2u;
    parameter.name = cm_hir_intern(&context, "U");
    parameter.span = test_span(16u, 17u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    parameter.index = 3u;
    parameter.name = cm_hir_intern(&context, "V");
    parameter.span = test_span(18u, 19u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    parameter.kind = CM_HIR_GENERIC_CONST;
    parameter.index = 4u;
    parameter.name = cm_hir_intern(&context, "K");
    parameter.declared_type = usize_type;
    parameter.span = test_span(20u, 21u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    parameter.index = 5u;
    parameter.name = cm_hir_intern(&context, "L");
    ...
    init_test_item(&item, CM_HIR_ITEM_TRAIT, bound_definition, root_id,
        ...
        test_span(10u, 80u));
    item.generic_parameter_start = bound_parameter_start;
    item.generic_parameter_count = 6u;
    item.data.trait_item.safety = CM_HIR_SAFE;
    assert(cm_hir_add_item(&context, &item, &item_id) == CM_HIR_OK);
    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TYPE_ALIAS, test_span(24u, 30u),
        &bound_item_definition) == CM_HIR_OK);
    init_test_item(&item, CM_HIR_ITEM_TYPE_ALIAS, bound_item_definition,
        root_id, bound_definition, cm_hir_intern(&context, "Item"),
        test_span(24u, 30u));
    item.data.type_alias_item.target = CM_HIR_TYPE_NONE;
    assert(cm_hir_add_item(&context, &item, &item_id) == CM_HIR_OK);

    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TRAIT, test_span(90u, 170u), &owner_definition)
        == CM_HIR_OK);
    memset(&parameter, 0, sizeof(parameter));
    parameter.kind = CM_HIR_GENERIC_LIFETIME;
    parameter.owner = owner_definition;
    parameter.name = cm_hir_intern(&context, "outer");
    parameter.span = test_span(92u, 93u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &owner_parameter_start) == CM_HIR_OK);
    parameter.kind = CM_HIR_GENERIC_TYPE;
    parameter.index = 1u;
    parameter.name = cm_hir_intern(&context, "Outer");
    ...
    parameter.declared_type = usize_type;
    parameter.span = test_span(96u, 97u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    ...
    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TYPE_ALIAS, test_span(110u, 160u), &gat_definition)
        == CM_HIR_OK);
    memset(&parameter, 0, sizeof(parameter));
    parameter.kind = CM_HIR_GENERIC_LIFETIME;
    parameter.owner = gat_definition;
    parameter.name = cm_hir_intern(&context, "a");
    parameter.span = test_span(112u, 113u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &gat_parameter_start) == CM_HIR_OK);
    parameter.kind = CM_HIR_GENERIC_TYPE;
    parameter.index = 1u;
    parameter.name = cm_hir_intern(&context, "T");
    parameter.span = test_span(114u, 115u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    parameter.kind = CM_HIR_GENERIC_CONST;
    parameter.index = 2u;
    ...
    parameter.declared_type = usize_type;
    parameter.span = test_span(116u, 117u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &parameter_id) == CM_HIR_OK);
    memset(&type, 0, sizeof(type));
    type.kind = CM_HIR_TYPE_PARAMETER_KIND;
    type.span = test_span(120u, 121u);
    type.data.parameter_type.parameter = owner_parameter_start + 1u;
    assert(cm_hir_add_type(&context, &type, &owner_type) == CM_HIR_OK);
    type.span = test_span(122u, 123u);
    type.data.parameter_type.parameter = gat_parameter_start + 1u;
    assert(cm_hir_add_type(&context, &type, &gat_type) == CM_HIR_OK);
    memset(arguments, 0, sizeof(arguments));
    arguments[0].kind = CM_HIR_GENERIC_ARG_LIFETIME;
    arguments[0].data.lifetime.kind = CM_HIR_REGION_EARLY_BOUND;
    arguments[0].data.lifetime.data.parameter = gat_parameter_start;
    arguments[1].kind = CM_HIR_GENERIC_ARG_LIFETIME;
    arguments[1].data.lifetime.kind = CM_HIR_REGION_EARLY_BOUND;
    arguments[1].data.lifetime.data.parameter = owner_parameter_start;
    arguments[2].kind = CM_HIR_GENERIC_ARG_TYPE;
    arguments[2].data.type = gat_type;
    arguments[3].kind = CM_HIR_GENERIC_ARG_TYPE;
    arguments[3].data.type = owner_type;
    arguments[4].kind = CM_HIR_GENERIC_ARG_CONST;
    arguments[4].data.constant.kind = CM_HIR_CONST_PARAMETER;
    arguments[4].data.constant.type = usize_type;
    arguments[4].data.constant.data.parameter = gat_parameter_start + 2u;
    arguments[5].kind = CM_HIR_GENERIC_ARG_CONST;
    arguments[5].data.constant.kind = CM_HIR_CONST_PARAMETER;
    arguments[5].data.constant.type = usize_type;
    arguments[5].data.constant.data.parameter = owner_parameter_start + 2u;
    memset(&bound, 0, sizeof(bound));
    bound.trait_type.definition = bound_definition;
    bound.trait_type.arguments = arguments;
    bound.trait_type.argument_count = 6u;
    memset(&equality, 0, sizeof(equality));
    equality.associated_type = bound_item_definition;
    equality.value = gat_type;
    equality.span = test_span(140u, 145u);
    bound.equalities = &equality;
    bound.equality_count = 1u;
    bound.span = test_span(120u, 150u);
    bound.modifier = CM_HIR_ASSOC_BOUND_REQUIRED;
    init_test_item(&item, CM_HIR_ITEM_TYPE_ALIAS, gat_definition, root_id,
        owner_definition, cm_hir_intern(&context, "Assoc"),
        test_span(110u, 160u));
    item.generic_parameter_start = gat_parameter_start;
    item.generic_parameter_count = 3u;
    item.data.type_alias_item.target = CM_HIR_TYPE_NONE;
    item.data.type_alias_item.bounds = &bound;
    item.data.type_alias_item.bound_count = 1u;
    assert(cm_hir_add_item(&context, &item, &gat_item_id) == CM_HIR_OK);
    stored = cm_hir_get_item(&context, gat_item_id);
    assert(stored != NULL
        && stored->data.type_alias_item.bound_count == 1u
        && stored->data.type_alias_item.bounds[0].trait_type.argument_count
            == 6u
        && stored->data.type_alias_item.bounds[0].trait_type.arguments[0]
                .data.lifetime.data.parameter == gat_parameter_start
        && stored->data.type_alias_item.bounds[0].trait_type.arguments[1]
                .data.lifetime.data.parameter == owner_parameter_start
        && stored->data.type_alias_item.bounds[0].trait_type.arguments[4]
                .data.constant.data.parameter == gat_parameter_start + 2u
        && stored->data.type_alias_item.bounds[0].trait_type.arguments[5]
                .data.constant.data.parameter == owner_parameter_start + 2u
        && stored->data.type_alias_item.bounds[0].equality_count == 1u
        && stored->data.type_alias_item.bounds[0].equalities[0].value
            == gat_type);
    stored_type = cm_hir_get_type(&context,
        stored->data.type_alias_item.bounds[0].trait_type.arguments[2]
            .data.type);
    assert(stored_type != NULL
        && stored_type->kind == CM_HIR_TYPE_PARAMETER_KIND
        && stored_type->data.parameter_type.parameter
            == gat_parameter_start + 1u);
    stored_type = cm_hir_get_type(&context,
        stored->data.type_alias_item.bounds[0].trait_type.arguments[3]
            .data.type);
    assert(stored_type != NULL
        && stored_type->kind == CM_HIR_TYPE_PARAMETER_KIND
        && stored_type->data.parameter_type.parameter
            == owner_parameter_start + 1u);

    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TRAIT, test_span(180u, 210u), &unrelated_definition)
        == CM_HIR_OK);
    memset(&parameter, 0, sizeof(parameter));
    parameter.kind = CM_HIR_GENERIC_TYPE;
    parameter.owner = unrelated_definition;
    parameter.name = cm_hir_intern(&context, "Foreign");
    parameter.span = test_span(182u, 183u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &unrelated_parameter) == CM_HIR_OK);
    init_test_item(&item, CM_HIR_ITEM_TRAIT, unrelated_definition, root_id,
        cm_hir_def_id_none(), cm_hir_intern(&context, "Unrelated"),
        test_span(180u, 210u));
    item.generic_parameter_start = unrelated_parameter;
    item.generic_parameter_count = 1u;
    item.data.trait_item.safety = CM_HIR_SAFE;
    assert(cm_hir_add_item(&context, &item, &item_id) == CM_HIR_OK);
    memset(&type, 0, sizeof(type));
    type.kind = CM_HIR_TYPE_PARAMETER_KIND;
    type.span = test_span(184u, 185u);
    type.data.parameter_type.parameter = unrelated_parameter;
    assert(cm_hir_add_type(&context, &type, &unrelated_type) == CM_HIR_OK);
    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TYPE_ALIAS, test_span(220u, 240u),
        &rejected_definition) == CM_HIR_OK);
    arguments[0].data.lifetime.data.parameter = owner_parameter_start;
    arguments[2].data.type = owner_type;
    arguments[3].data.type = unrelated_type;
    arguments[4].data.constant.data.parameter = owner_parameter_start + 2u;
    ...
    arguments[3].data.type = owner_type;
    equality.value = unrelated_type;
    assert(cm_hir_add_item(&context, &item, &item_id) == CM_HIR_INVALID_ID);
    assert(item_id == CM_HIR_ITEM_NONE && context.items.len == item_count
        && cm_arena_bytes_used(&context.storage) == arena_bytes);

    assert(cm_hir_reserve_item_definition_as(&context, crate_id,
        CM_HIR_ITEM_TYPE_ALIAS, test_span(250u, 280u),
        &mismatched_definition) == CM_HIR_OK);
    memset(&parameter, 0, sizeof(parameter));
    parameter.kind = CM_HIR_GENERIC_CONST;
    parameter.owner = mismatched_definition;
    parameter.name = cm_hir_intern(&context, "Small");
    parameter.declared_type = u8_type;
    parameter.span = test_span(252u, 253u);
    assert(cm_hir_add_generic_param(&context, &parameter,
        &mismatched_parameter) == CM_HIR_OK);
    arguments[3].data.type = owner_type;
    arguments[4].data.constant.type = u8_type;
    arguments[4].data.constant.data.parameter = mismatched_parameter;
    equality.value = owner_type;
    init_test_item(&item, CM_HIR_ITEM_TYPE_ALIAS, mismatched_definition,
        root_id, owner_definition, cm_hir_intern(&context, "Mismatch"),
        test_span(250u, 280u));
    item.generic_parameter_start = mismatched_parameter;
    item.generic_parameter_count = 1u;
    item.data.type_alias_item.target = CM_HIR_TYPE_NONE;
    item.data.type_alias_item.bounds = &bound;
    item.data.type_alias_item.bound_count = 1u;
    ...
    assert(item_id == CM_HIR_ITEM_NONE && context.items.len == item_count
        && cm_arena_bytes_used(&context.storage) == arena_bytes);
    stored_parameter = cm_hir_get_generic_param(&context,
        mismatched_parameter);
    assert(stored_parameter != NULL
        && stored_parameter->kind == CM_HIR_GENERIC_CONST
        && stored_parameter->declared_type == u8_type);
...
static void test_auto_trait_and_negative_impl_model(void)
{
    ...
    item.data.impl_item.self_type = u8_type;
    item.data.impl_item.has_trait = 1;
    item.data.impl_item.is_negative = 2;
    item.data.impl_item.safety = CM_HIR_SAFE;
    ...
    assert(stored != NULL && stored->kind == CM_HIR_ITEM_IMPL
        && stored->data.impl_item.is_negative
...
int main(void)
{
    ...
    test_gat_bound_generic_scope_model_invariants();
