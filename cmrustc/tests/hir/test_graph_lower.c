...
static void test_rustc_as_vec_into_iter_fixture(void)
{
    ...
    check(hir_dump != NULL
        ...
        "AsVecIntoIter hir-v34 dump omitted the attributed reexport");
    free(hir_dump);
    ...
    check(hir_dump != NULL
        ...
        "no-global-oom hir-v34 dump retained a cfg-disabled import");
    free(hir_dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
...
static void test_rustc_future_ready_pending_fixture(void)
{
    ...
    check(hir_dump != NULL
        ...
        && strstr(hir_dump,
            ...
            != NULL,
        "Future/Ready/Pending hir-v34 dump lost binding distinctions");
    free(hir_dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
...
static void test_rustc_try_family_fixture(void)
{
    ...
    check(hir_dump != NULL
        ...
        "Try-family hir-v34 dump omitted its supertrait or methods");
    free(hir_dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
...
static void test_rustc_into_iterator_deref_fixture(void)
{
    ...
    check(hir_dump != NULL
        ...
        "IntoIterator/Deref hir-v34 bound/equality dump order differs");
    free(hir_dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
...
static void test_rustc_iterator_methods_fixture(void)
{
    ...
    dump_order_ok = dump_order_ok
        && text_count_between(hir_dump, dump_end, "trait-predicate item#")
            == 9u;
    check(dump_order_ok,
        "Iterator hir-v34 dump lost item, attribute, or predicate order");
    free(hir_dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
...
static void test_rustc_iterator_generic_methods_fixture(void)
{
...

    {
        ...
        check(dump_ok,
            "generic Iterator hir-v34 dump is incomplete or nondeterministic");
        free(dump_a);
        free(dump_b);
    }
    cm_hir_module_map_destroy(&map);
...
static void test_enum_variant_glob_import(void)
{
    ...
    dump = dump_hir(&hir);
    check(dump != NULL
        && strncmp(dump, "hir-v34\n", strlen("hir-v34\n")) == 0
        && strstr(dump, "enum-variant bound enum-item#1 variant=0")
            != NULL,
        "hir-v34 dump omitted canonical enum variant identity");
    free(dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
...
static void test_core_auto_trait_negative_impl_cluster(void)
{
    ...
    check(dump != NULL
        ...
        "hir-v34 dump erased auto-trait or negative-impl headers");
    free(dump);
    cm_hir_module_map_destroy(&map);
    cm_hir_context_destroy(&hir);
    cm_module_graph_destroy(&graph);
