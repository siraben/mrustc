...
static const CmHirItem *find_root_item(const CmHirContext *context,
    const char *name)
{
    size_t index;
    size_t length;
...
static void test_unsupported_constructs_are_errors(void)
{
    ...
    result = lower_source(
        "trait Error {} fn make() -> impl Error {}", &context, NULL);
    assert(result.error_count == 1u);
    assert(result.first_error.kind == CM_HIR_LOWER_UNSUPPORTED_TYPE);
    ...
    result = lower_source(
        "fn life<'a>(value: impl 'a) {}", &context, NULL);
    assert(result.error_count == 1u);
    assert(result.first_error.kind == CM_HIR_LOWER_INVALID_AST);
    assert(strstr(result.first_error.message,
        "at least one trait bound") != NULL);
    cm_hir_context_destroy(&context);
...
static void test_gat_associated_type_bound_record_rollback(void)
{
    static const char source[] =
        "trait Bound<'x, U> {}"
        "trait Owner<'outer, Outer> {"
        " type Assoc<'a, T>:"
        " Bound<'a, T> + Bound<'missing, T>;"
        "}";
    const CmAstItemId *root_id;
    CmAst ast;
    CmParseResult parse_result;
    ...
    CmAstItem *associated;
    CmAstAssociatedTypeBound *bounds;
    CmAstTypeId saved_first_trait_type;
    CmHirLowerOptions options;
    CmHirContext first_context;
    CmHirContext second_context;
    CmHirLowerResult first_result;
    CmHirLowerResult second_result;
    const CmHirItem *first_owner;
    const CmHirItem *second_owner;
    ...
    assert(parse_result.error_count == 0u && bounds != NULL
        ...
        && bounds[1].kind == CM_AST_ASSOC_BOUND_TRAIT);

    cm_hir_lower_options_init(&options);
    options.crate_name = "gat_associated_bound_record_rollback";
    options.source = 7u;
    saved_first_trait_type = bounds[0].trait_type;
    bounds[0].trait_type = bounds[1].trait_type;
    cm_hir_context_init(&first_context);
    first_result = cm_hir_lower_crate(&first_context, &ast, &options);
    bounds[0].trait_type = saved_first_trait_type;
    cm_hir_context_init(&second_context);
    second_result = cm_hir_lower_crate(&second_context, &ast, &options);
    first_owner = find_root_item(&first_context, "Owner");
    second_owner = find_root_item(&second_context, "Owner");
    ...
    assert(first_result.error_count == 1u
        ...
        && strstr(first_result.first_error.message,
            "undeclared lifetime") != NULL
        && strstr(second_result.first_error.message,
            "undeclared lifetime") != NULL
        && first_owner != NULL && second_owner != NULL
        && find_child(&first_context, first_owner->definition, "Assoc")
            == NULL
        && find_child(&second_context, second_owner->definition, "Assoc")
            == NULL
        && first_context.crates.len == second_context.crates.len
        && first_context.modules.len == second_context.modules.len
        && first_context.items.len == second_context.items.len
...
static void test_argument_impl_trait_lowers(void)
{
    static const char source[] =
        "trait Owner {"
        " fn consume<'a, T>(&self,"
        "  first: impl Bound<T> + 'a,"
        "  second: &impl for<'b> Bound<&'b T> + ?Sized,"
        "  third: impl ~const Send) -> &Self;"
        "}"
        "trait Bound<A> {} trait Send {} trait Sized {}";
    const CmHirItem *owner;
    const CmHirItem *bound;
    const CmHirItem *send;
    const CmHirItem *method;
    const CmHirGenericParam *first_apit;
    const CmHirGenericParam *second_apit;
    const CmHirGenericParam *third_apit;
    ...
    send = find_item(&context, "Send");
    method = owner == NULL ? NULL
        : find_child(&context, owner->definition, "consume");
    first_apit = method == NULL || method->generic_parameter_count != 5u
        ? NULL : cm_hir_get_generic_param(&context,
            method->generic_parameter_start + 2u);
    second_apit = method == NULL || method->generic_parameter_count != 5u
        ? NULL : cm_hir_get_generic_param(&context,
            method->generic_parameter_start + 3u);
    third_apit = method == NULL || method->generic_parameter_count != 5u
        ? NULL : cm_hir_get_generic_param(&context,
            method->generic_parameter_start + 4u);
    first_type = method == NULL
            || method->data.function_item.signature.parameter_count != 4u
        ? NULL : cm_hir_get_type(&context,
            method->data.function_item.signature.parameters[1].type);
    second_type = method == NULL
            || method->data.function_item.signature.parameter_count != 4u
        ? NULL : cm_hir_get_type(&context,
            method->data.function_item.signature.parameters[2].type);
    second_pointee = second_type == NULL
            || second_type->kind != CM_HIR_TYPE_REFERENCE_KIND
        ? NULL : cm_hir_get_type(&context,
            second_type->data.reference_type.pointee);
    third_type = method == NULL
            || method->data.function_item.signature.parameter_count != 4u
        ? NULL : cm_hir_get_type(&context,
            method->data.function_item.signature.parameters[3].type);
    first_subject = method == NULL || method->predicate_count != 3u
        ? NULL : cm_hir_get_type(&context, method->predicates[0].subject);
    second_subject = method == NULL || method->predicate_count != 3u
        ? NULL : cm_hir_get_type(&context, method->predicates[1].subject);
    third_subject = method == NULL || method->predicate_count != 3u
        ? NULL : cm_hir_get_type(&context, method->predicates[2].subject);

    assert(result.error_count == 0u
        && owner != NULL && bound != NULL && send != NULL
        && method != NULL && method->kind == CM_HIR_ITEM_FUNCTION
        && method->generic_parameter_count == 5u
        && first_apit != NULL && first_apit->kind == CM_HIR_GENERIC_TYPE
        && first_apit->index == 2u
        && cm_hir_def_id_equal(first_apit->owner, method->definition)
        && hir_string_is(&context, first_apit->name, "$APIT0")
        && !first_apit->is_relaxed_sized
        && second_apit != NULL && second_apit->index == 3u
        && cm_hir_def_id_equal(second_apit->owner, method->definition)
        && hir_string_is(&context, second_apit->name, "$APIT1")
        && second_apit->is_relaxed_sized
        && third_apit != NULL && third_apit->index == 4u
        && cm_hir_def_id_equal(third_apit->owner, method->definition)
        && hir_string_is(&context, third_apit->name, "$APIT2")
        && first_type != NULL
        && first_type->kind == CM_HIR_TYPE_PARAMETER_KIND
        && first_type->data.parameter_type.parameter
            == method->generic_parameter_start + 2u
        && second_pointee != NULL
        && second_pointee->kind == CM_HIR_TYPE_PARAMETER_KIND
        && second_pointee->data.parameter_type.parameter
            == method->generic_parameter_start + 3u
        && third_type != NULL
        && third_type->kind == CM_HIR_TYPE_PARAMETER_KIND
        && third_type->data.parameter_type.parameter
            == method->generic_parameter_start + 4u
        ...
        && cm_hir_def_id_equal(method->predicates[0]
                .trait_type.definition, bound->definition)
        && second_subject != NULL
        && second_subject->kind == CM_HIR_TYPE_PARAMETER_KIND
        && second_subject->data.parameter_type.parameter
            == method->generic_parameter_start + 3u
        && cm_hir_def_id_equal(method->predicates[1]
                .trait_type.definition, bound->definition)
        && method->predicates[1].binder.lifetime_count == 1u
        && hir_string_is(&context,
            method->predicates[1].binder.lifetimes[0], "'b")
        && third_subject != NULL
        && third_subject->kind == CM_HIR_TYPE_PARAMETER_KIND
        && third_subject->data.parameter_type.parameter
            == method->generic_parameter_start + 4u
        && cm_hir_def_id_equal(method->predicates[2]
                .trait_type.definition, send->definition)
        && method->predicates[2].modifier
            == CM_HIR_PREDICATE_CONST_IF_CONST
        && method->outlives_predicate_count == 1u
        && method->outlives_predicates[0].subject_kind
            == CM_HIR_OUTLIVES_TYPE
    ...
    assert(result.error_count == 0u && method != NULL
        ...
        && method->predicate_count == 3u
        && method->outlives_predicate_count == 1u
        && second_apit != NULL && second_apit->is_relaxed_sized
        && hir_string_is(&context, second_apit->name, "$APIT1"));
    cm_hir_context_destroy(&context);
    cm_expanded_ast_destroy(&expanded);
    cm_ast_destroy(&ast);
...
static void test_argument_impl_trait_method_parity(void)
{
    static const char source[] =
        ...
        " fn consume(value: impl Send) {}"
        "}"
        "trait Consumer { fn consume(value: impl Send); }"
        "trait Send {}";
    const CmHirItem *consumer;
    const CmHirItem *impl_item;
    const CmHirItem *trait_method;
    const CmHirItem *impl_method;
    const CmHirType *trait_parameter;
    const CmHirType *impl_parameter;
    CmHirContext context;
    CmHirLowerResult result;
    ...
    impl_item = find_impl(&context);
    trait_method = consumer == NULL ? NULL
        : find_child(&context, consumer->definition, "consume");
    impl_method = impl_item == NULL ? NULL
        : find_child(&context, impl_item->definition, "consume");
    trait_parameter = trait_method == NULL
            || trait_method->data.function_item.signature.parameter_count
                != 1u
        ? NULL : cm_hir_get_type(&context, trait_method->data.function_item
            .signature.parameters[0].type);
    impl_parameter = impl_method == NULL
            || impl_method->data.function_item.signature.parameter_count
                != 1u
        ? NULL : cm_hir_get_type(&context, impl_method->data.function_item
            .signature.parameters[0].type);
    assert(result.error_count == 0u && consumer != NULL
        && impl_item != NULL && trait_method != NULL && impl_method != NULL
        && trait_method->generic_parameter_count == 1u
        && impl_method->generic_parameter_count == 1u
        && trait_parameter != NULL
        && trait_parameter->kind == CM_HIR_TYPE_PARAMETER_KIND
        && trait_parameter->data.parameter_type.parameter
            == trait_method->generic_parameter_start
        && impl_parameter != NULL
        && impl_parameter->kind == CM_HIR_TYPE_PARAMETER_KIND
        && impl_parameter->data.parameter_type.parameter
...
{
    static const char source[] =
        "trait Send {} struct Wrapper<T>;"
        "fn ordinary(value: Wrapper<u8>) {}"
        "fn nested(value: Wrapper<impl Send>) {}";
    const CmHirItem *send;
    const CmHirItem *ordinary;
    const CmHirItem *nested;
    const CmHirType *ordinary_type;
    const CmHirType *nested_type;
    ...
    result = lower_source(source, &context, NULL);
    send = find_item(&context, "Send");
    ordinary = find_item(&context, "ordinary");
    nested = find_item(&context, "nested");
    ordinary_type = ordinary == NULL
            || ordinary->data.function_item.signature.parameter_count != 1u
        ? NULL : cm_hir_get_type(&context,
            ordinary->data.function_item.signature.parameters[0].type);
    nested_type = nested == NULL
            || nested->data.function_item.signature.parameter_count != 1u
        ? NULL : cm_hir_get_type(&context,
            nested->data.function_item.signature.parameters[0].type);
    nested_argument = nested_type == NULL
            || nested_type->kind != CM_HIR_TYPE_ADT_KIND
            || nested_type->data.named_type.argument_count != 1u
            || nested_type->data.named_type.arguments == NULL
            || nested_type->data.named_type.arguments[0].kind
                != CM_HIR_GENERIC_ARG_TYPE
        ? NULL : cm_hir_get_type(&context,
            nested_type->data.named_type.arguments[0].data.type);
    assert(result.error_count == 0u && send != NULL
        && ordinary != NULL && ordinary->generic_parameter_count == 0u
        && ordinary_type != NULL
        && ordinary_type->kind == CM_HIR_TYPE_ADT_KIND
        && nested != NULL && nested->generic_parameter_count == 1u
        && nested_argument != NULL
        && nested_argument->kind == CM_HIR_TYPE_PARAMETER_KIND
        && nested_argument->data.parameter_type.parameter
            == nested->generic_parameter_start
        && nested->predicate_count == 1u
        && cm_hir_def_id_equal(nested->predicates[0]
                .trait_type.definition, send->definition));
    cm_hir_context_destroy(&context);
}
...
static void test_higher_ranked_impl_trait_fails_closed(void)
{
    ...
    CmAstTypeBound *saved_bounds;
    uint32_t saved_bound_count;
    ...
    saved_bounds = opaque->bounds;
    saved_bound_count = opaque->bound_count;

    opaque->bounds = NULL;
    expect_invalid_ast_lowering(&ast, &options,
        "opaque impl trait type has no bound storage");
    opaque->bounds = saved_bounds;

    opaque->bound_count = 0u;
    expect_invalid_ast_lowering(&ast, &options,
        "opaque impl trait type has no bound storage");
    opaque->bound_count = saved_bound_count;
...
static void check_gat_associated_type_bound_scope(
    const CmHirContext *context)
{
    const CmHirItem *owner;
    const CmHirItem *associated;
    ...
    const CmHirItem *searcher_associated;
    const CmHirItem *searcher_trait;
    const CmHirGenericParam *owner_lifetime;
    const CmHirGenericParam *owner_type;
    const CmHirGenericParam *owner_const;
    const CmHirGenericParam *associated_lifetime;
    const CmHirGenericParam *associated_type;
    const CmHirGenericParam *associated_const;
    const CmHirGenericParam *bound_const;
    const CmHirGenericParam *bound_second_const;
    const CmHirGenericParam *searcher_lifetime;
    const CmHirAssociatedTypeBound *bound;
    const CmHirAssociatedTypeBound *searcher_bound;
    const CmHirType *type_argument;
    const CmHirType *outer_type_argument;
    const CmHirType *declared_type;

    owner = find_root_item(context, "Owner");
    associated = owner == NULL ? NULL
        : find_child(context, owner->definition, "Assoc");
    bound_trait = find_root_item(context, "Bound");
    pattern = find_root_item(context, "Pattern");
    searcher_associated = pattern == NULL ? NULL
        : find_child(context, pattern->definition, "Searcher");
    searcher_trait = find_root_item(context, "Searcher");
    assert(owner != NULL && owner->kind == CM_HIR_ITEM_TRAIT
        && owner->generic_parameter_count == 3u
        && associated != NULL
        && associated->kind == CM_HIR_ITEM_TYPE_ALIAS
        && associated->generic_parameter_count == 3u
        && bound_trait != NULL && bound_trait->kind == CM_HIR_ITEM_TRAIT
        && bound_trait->generic_parameter_count == 6u
        && pattern != NULL && pattern->kind == CM_HIR_ITEM_TRAIT
        && searcher_associated != NULL
        && searcher_associated->kind == CM_HIR_ITEM_TYPE_ALIAS
        && searcher_associated->generic_parameter_count == 1u
        && searcher_trait != NULL
        && searcher_trait->kind == CM_HIR_ITEM_TRAIT
        && searcher_trait->generic_parameter_count == 1u);

    owner_lifetime = cm_hir_get_generic_param(context,
        owner->generic_parameter_start);
    owner_type = cm_hir_get_generic_param(context,
        owner->generic_parameter_start + 1u);
    owner_const = cm_hir_get_generic_param(context,
        owner->generic_parameter_start + 2u);
    associated_lifetime = cm_hir_get_generic_param(context,
        associated->generic_parameter_start);
    associated_type = cm_hir_get_generic_param(context,
        associated->generic_parameter_start + 1u);
    associated_const = cm_hir_get_generic_param(context,
        associated->generic_parameter_start + 2u);
    bound_const = cm_hir_get_generic_param(context,
        bound_trait->generic_parameter_start + 4u);
    bound_second_const = cm_hir_get_generic_param(context,
        bound_trait->generic_parameter_start + 5u);
    searcher_lifetime = cm_hir_get_generic_param(context,
        searcher_associated->generic_parameter_start);
    assert(owner_lifetime != NULL
        && owner_lifetime->kind == CM_HIR_GENERIC_LIFETIME
        && cm_hir_def_id_equal(owner_lifetime->owner, owner->definition)
        && hir_string_is(context, owner_lifetime->name, "'outer")
        && owner_type != NULL && owner_type->kind == CM_HIR_GENERIC_TYPE
        && cm_hir_def_id_equal(owner_type->owner, owner->definition)
        && hir_string_is(context, owner_type->name, "Outer")
        && owner_const != NULL
        && owner_const->kind == CM_HIR_GENERIC_CONST
        && cm_hir_def_id_equal(owner_const->owner, owner->definition)
        && hir_string_is(context, owner_const->name, "M")
        && associated_lifetime != NULL
        && associated_lifetime->kind == CM_HIR_GENERIC_LIFETIME
        && cm_hir_def_id_equal(associated_lifetime->owner,
            associated->definition)
        && hir_string_is(context, associated_lifetime->name, "'a")
        && associated_type != NULL
        && associated_type->kind == CM_HIR_GENERIC_TYPE
        && cm_hir_def_id_equal(associated_type->owner,
            associated->definition)
        && hir_string_is(context, associated_type->name, "T")
        && associated_const != NULL
        && associated_const->kind == CM_HIR_GENERIC_CONST
        && cm_hir_def_id_equal(associated_const->owner,
            associated->definition)
        && hir_string_is(context, associated_const->name, "N")
        && bound_const != NULL
        && bound_const->kind == CM_HIR_GENERIC_CONST
        && cm_hir_def_id_equal(bound_const->owner,
            bound_trait->definition)
        && bound_second_const != NULL
        && bound_second_const->kind == CM_HIR_GENERIC_CONST
        && cm_hir_def_id_equal(bound_second_const->owner,
            bound_trait->definition)
        && searcher_lifetime != NULL
        && searcher_lifetime->kind == CM_HIR_GENERIC_LIFETIME
        && cm_hir_def_id_equal(searcher_lifetime->owner,
            searcher_associated->definition));

    declared_type = cm_hir_get_type(context, owner_const->declared_type);
    assert(declared_type != NULL
        && declared_type->kind == CM_HIR_TYPE_INTEGER_KIND
        && declared_type->data.integer_type.kind == CM_HIR_INT_USIZE);
    declared_type = cm_hir_get_type(context,
        associated_const->declared_type);
    assert(declared_type != NULL
        && declared_type->kind == CM_HIR_TYPE_INTEGER_KIND
        && declared_type->data.integer_type.kind == CM_HIR_INT_USIZE);
    declared_type = cm_hir_get_type(context, bound_const->declared_type);
    assert(declared_type != NULL
        && declared_type->kind == CM_HIR_TYPE_INTEGER_KIND
        && declared_type->data.integer_type.kind == CM_HIR_INT_USIZE);
    declared_type = cm_hir_get_type(context,
        bound_second_const->declared_type);
    assert(declared_type != NULL
        && declared_type->kind == CM_HIR_TYPE_INTEGER_KIND
        && declared_type->data.integer_type.kind == CM_HIR_INT_USIZE);

    bound = associated->data.type_alias_item.bound_count == 1u
        ? &associated->data.type_alias_item.bounds[0] : NULL;
    assert(bound != NULL
        && cm_hir_def_id_equal(bound->trait_type.definition,
            bound_trait->definition)
        && bound->trait_type.argument_count == 6u
        ...
        && bound->trait_type.arguments[1].kind
            == CM_HIR_GENERIC_ARG_LIFETIME
        && bound->trait_type.arguments[1].data.lifetime.kind
            == CM_HIR_REGION_EARLY_BOUND
        && bound->trait_type.arguments[1].data.lifetime.data.parameter
            == owner->generic_parameter_start
        && bound->trait_type.arguments[2].kind == CM_HIR_GENERIC_ARG_TYPE
        && bound->trait_type.arguments[3].kind == CM_HIR_GENERIC_ARG_TYPE
        && bound->trait_type.arguments[4].kind == CM_HIR_GENERIC_ARG_CONST
        && bound->trait_type.arguments[4].data.constant.kind
            == CM_HIR_CONST_PARAMETER
        && bound->trait_type.arguments[4].data.constant.data.parameter
            == associated->generic_parameter_start + 2u
        && bound->trait_type.arguments[4].data.constant.type
            == associated_const->declared_type
        && bound->trait_type.arguments[5].kind == CM_HIR_GENERIC_ARG_CONST
        && bound->trait_type.arguments[5].data.constant.kind
            == CM_HIR_CONST_PARAMETER
        && bound->trait_type.arguments[5].data.constant.data.parameter
            == owner->generic_parameter_start + 2u
        && bound->trait_type.arguments[5].data.constant.type
            == owner_const->declared_type);
    type_argument = cm_hir_get_type(context,
        bound->trait_type.arguments[2].data.type);
    outer_type_argument = cm_hir_get_type(context,
        bound->trait_type.arguments[3].data.type);
    assert(type_argument != NULL
        && type_argument->kind == CM_HIR_TYPE_PARAMETER_KIND
        && type_argument->data.parameter_type.parameter
            == associated->generic_parameter_start + 1u
        && outer_type_argument != NULL
        && outer_type_argument->kind == CM_HIR_TYPE_PARAMETER_KIND
        && outer_type_argument->data.parameter_type.parameter
            == owner->generic_parameter_start + 1u);

    searcher_bound = searcher_associated->data.type_alias_item.bound_count
            == 1u
        ? &searcher_associated->data.type_alias_item.bounds[0] : NULL;
    assert(searcher_bound != NULL
        && cm_hir_def_id_equal(searcher_bound->trait_type.definition,
            searcher_trait->definition)
        && searcher_bound->trait_type.argument_count == 1u
        && searcher_bound->trait_type.arguments != NULL
        && searcher_bound->trait_type.arguments[0].kind
            == CM_HIR_GENERIC_ARG_LIFETIME
        && searcher_bound->trait_type.arguments[0].data.lifetime.kind
            == CM_HIR_REGION_EARLY_BOUND
        && searcher_bound->trait_type.arguments[0].data.lifetime
                .data.parameter
            == searcher_associated->generic_parameter_start);
}

static void test_gat_associated_type_bound_scope(void)
{
    static const char source[] =
        ...
        "trait Pattern { type Searcher<'a>: Searcher<'a>; }"
        "trait Searcher<'a> {}";
    CmAst ast;
    CmExpandedAst expanded;
    CmHirContext context;
    ...
    result = lower_source(source, &context, NULL);
    if (result.error_count != 0u) {
        fprintf(stderr, "raw GAT bound scope lowering failed: %s: %s\n",
            cm_hir_lower_error_kind_name(result.first_error.kind),
            result.first_error.message);
    }
    assert(result.error_count == 0u);
    check_gat_associated_type_bound_scope(&context);
    cm_hir_context_destroy(&context);

    make_cfg_view(source, &ast, &expanded);
    result = lower_cfg_view(&context, &ast, &expanded);
    if (result.error_count != 0u) {
        fprintf(stderr, "cfg GAT bound scope lowering failed: %s: %s\n",
            cm_hir_lower_error_kind_name(result.first_error.kind),
            result.first_error.message);
    }
    assert(result.error_count == 0u);
...
static void test_gat_associated_type_bound_scope_errors(void)
{
    static const char *const rejected[] = {
        "trait Owner { type Assoc<T>: Bound<T>; } trait Bound<'a> {}",
        "trait Owner { type Assoc<'a>: Bound<'a>; } "
            "trait Bound<'x, T> {}",
        "trait Owner { type Assoc<T>: Bound<T>; } "
            "trait Bound<const K: usize> {}",
        "trait Owner { type Assoc<const N: usize>: Bound<N>; } "
            "trait Bound<const K: u8> {}",
        "trait Owner { type Assoc<'a>: Bound<'_>; } "
            "trait Bound<'x> {}",
        "trait Owner { type Assoc<'a>: Bound<'missing>; } "
            "trait Bound<'x> {}"
    };
    static const CmHirLowerErrorKind rejected_kinds[] = {
        CM_HIR_LOWER_UNSUPPORTED_GENERIC,
    ...
    static const char *const rejected_messages[] = {
        "argument kind differs",
        "omits a required type argument",
        "must name an authenticated const parameter in scope",
        "type differs from its target parameter",
        "authenticated early- or late-bound lifetime",
        "undeclared lifetime"
    };
    CmAst ast;
    CmExpandedAst expanded;
...
int main(void)
{
    ...
    test_gat_associated_type_bound_record_rollback();
    ...
    test_gat_associated_type_bound_scope();
    test_gat_associated_type_bound_scope_errors();
